...
#code form https://github.com/sharma-n/DAG_Scheduling/tree/master
# paper: A list scheduling algorithm for heterogeneous systems based on a critical node cost table and pessimistic cost table
#paper URL: https://onlinelibrary.wiley.com/doi/full/10.1002/cpe.3944
class Task:
...
class IPEFT:
    ...
    def outcome(self):
        temp=[[]]*len(self.processors)
        sch=defaultdict(list)
        for p in self.processors:
            for t in p.task_list:
                sch[p.id].append(t.id)
                # temp[p.id].append(t.id)
        temp = [list(values) for values in sch.values()]
        return temp
...
if __name__ == "__main__":
    ...
    # args = ap.parse_args()
    new_sch = IPEFT(file='test.dot', verbose=True, p=4, b=0.1, ccr=0.1)
    print(new_sch.outcome())